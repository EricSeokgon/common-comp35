...
<%
	...
	%>
		location.href="<c:url value='/uss/ion/tir/selectTwitterPopup.do'/>?cmd=<egovc:out value="<%=sCmd%>"/>&verify=Y&at=1";
	<%
	}else{
		//out.println("location.href='/uss/ion/tir/selectTwitterPopup.do?cmd="+sCmd+"&verify=Y';");
	%>
		location.href="<c:url value='/uss/ion/tir/selectTwitterPopup.do'/>?cmd=<egovc:out value="<%=sCmd%>"/>&verify=Y";
	<%
	}
%>
</script>
